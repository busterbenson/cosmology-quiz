 ...
 * Test Permalink
 * 
 * Test a specific permalink answer sequence to see the actual results
 */
...
async function testPermalink(answerString, verbose = false) {
  // Load data files
  ...
  if (verbose) {
    console.log(`🔍 TESTING PERMALINK: ${answerString}`);
  ...
  return {
    ...
    finalResults: results.slice(0, 15), // Show top 15
...
async function main() {
  ...
  try {
    const result = await testPermalink(answerString, false);
    ...
    // Find Analytical Idealism's actual rank
    const aiResult = result.finalResults.find(r => r.name === 'Analytical Idealism');
    if (aiResult) {
      const aiRank = result.finalResults.findIndex(r => r.name === 'Analytical Idealism') + 1;
      console.log(`\n⭐ Analytical Idealism: Rank #${aiRank}, Score ${aiResult.score}`);
    } else {
      console.log(`\n❌ Analytical Idealism not in top 15 results`);
    }
